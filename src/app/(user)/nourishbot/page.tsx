...
// export default function NourishBot() {
//   const [messages, setMessages] = useState<Msg[]>([
//     { role: "assistant", content: "Hi — I'm NourishBot. Ask me about leftovers, nutrition, budget meals, or food-waste tips." },
//   ]);
//   const [input, setInput] = useState("");
//   const [loading, setLoading] = useState(false);
//   const [sessionId, setSessionId] = useState<string | null>(typeof window !== "undefined" ? localStorage.getItem("nourish_session") : null);
...
//   useEffect(() => {
//     if (sessionId) localStorage.setItem("nourish_session", sessionId);
//   }, [sessionId]);
...
//     setLoading(true);
//     try {
//       const res = await fetch("/api/nourishbot", {
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({ sessionId, message: userText }),
//       });
//       const data = await res.json();
//       if (!res.ok) throw new Error(data.error || "NourishBot failed");
//       setSessionId(data.sessionId);
//       setMessages(prev => [...prev, { role: "assistant", content: data.reply }]);
//     } catch (err: any) {
//       console.error(err);
//       setMessages(prev => [...prev, { role: "assistant", content: "Sorry — I couldn't process that. Try again later." }]);
//     } finally {
//       setLoading(false);
//     }
//   };
...
//       <h2 className="text-2xl font-bold mb-4">NourishBot</h2>

//       <div className="h-80 overflow-y-auto border rounded-lg p-4 ">
//         {messages.map((m, i) => (
//           <div key={i} className={`mb-3 ${m.role === "assistant" ? "text-left" : "text-right"}`}>
//             <div className={`inline-block px-4 py-2 rounded-lg ${m.role === "assistant" ? " shadow" : ""}`}>
...
//           placeholder="Ask about reducing waste, planning meals, leftovers, or environmental impact..."
//         />
//         <button onClick={send} disabled={loading || !input.trim()} className={`px-4 py-2 rounded-lg font-semibold ${loading ? "bg-gray-300" : "bg-green-600 text-white"}`}>
